...
class PaymentOutboxPublisher:
    ...
    async def start(self):
        ...
        logger.info("PaymentOutboxPublisher background task started")
    ...
    async def stop(self):
        ...
        if self.task:
            ...
            try:
                ...
                pass
            logger.info("PaymentOutboxPublisher background task stopped")
    ...
    async def run_loop(self):
        while self.running:
            try:
                ...
                logger.error(f"Error in PaymentOutboxPublisher loop: {e}", exc_info=True)
                await asyncio.sleep(self.poll_interval)
            ...
            try:
                ...
                for event in events:
                    try:
                        ...
                        await payment_producer.publish_event(envelope)
                        event.status = "PUBLISHED"
                        event.published_at = datetime.now(timezone.utc)
                        event.last_error = None
                        logger.info(f"Relayed payment outbox event {event.id} ({event.event_type}) to Kafka")
                    except Exception as e:
                        event.retry_count += 1
                        event.last_error = str(e)
                        logger.warning(
                            f"Failed to publish payment outbox event {event.id}: {e} (retry {event.retry_count}/{self.max_retries})"
                        )
                        if event.retry_count >= self.max_retries:
                            event.status = "FAILED"
                            logger.error(f"Payment outbox event {event.id} exceeded max retries. Marked as FAILED.")
...
payment_outbox_publisher = PaymentOutboxPublisher()
